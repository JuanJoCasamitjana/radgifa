...
func (_q *MemberQuery) QueryQuestionnaire() *QuestionnaireQuery {
	query := (&QuestionnaireClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *MemberQuery) QueryAnswers() *AnswerQuery {
...
func (_q *MemberQuery) Clone() *MemberQuery {
	...
	return &MemberQuery{
		...
		withQuestionnaire: _q.withQuestionnaire.Clone(),
...
func (_q *MemberQuery) WithQuestionnaire(opts ...func(*QuestionnaireQuery)) *MemberQuery {
	query := (&QuestionnaireClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withQuestionnaire = query
	return _q
}
...
func (_q *MemberQuery) WithAnswers(opts ...func(*AnswerQuery)) *MemberQuery {
	query := (&AnswerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Member.Query().
//		GroupBy(member.FieldDisplayName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Member.Query().
//		Select(member.FieldDisplayName).
//		Scan(ctx, &v)
...
func (_q *MemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Member, error) {
	var (
		...
		loadedTypes = [3]bool{
			_q.withUser != nil,
			_q.withQuestionnaire != nil,
			_q.withAnswers != nil,
		}
	)
	if _q.withUser != nil || _q.withQuestionnaire != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withQuestionnaire; query != nil {
		if err := _q.loadQuestionnaire(ctx, query, nodes, nil,
			func(n *Member, e *Questionnaire) { n.Edges.Questionnaire = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAnswers; query != nil {
		if err := _q.loadAnswers(ctx, query, nodes,
			func(n *Member) { n.Edges.Answers = []*Answer{} },
			func(n *Member, e *Answer) { n.Edges.Answers = append(n.Edges.Answers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MemberQuery) loadQuestionnaire(ctx context.Context, query *QuestionnaireQuery, nodes []*Member, init func(*Member), assign func(*Member, *Questionnaire)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Member)
	for i := range nodes {
		if nodes[i].questionnaire_members == nil {
			continue
		}
		fk := *nodes[i].questionnaire_members
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "questionnaire_members" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *MemberQuery) loadAnswers(ctx context.Context, query *AnswerQuery, nodes []*Member, init func(*Member), assign func(*Member, *Answer)) error {
	...
	query.Where(predicate.Answer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(member.AnswersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.member_answers
		if fk == nil {
			return fmt.Errorf(`foreign-key "member_answers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "member_answers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
